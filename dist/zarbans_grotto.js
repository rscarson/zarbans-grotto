...
const _ = { Float: "Float", Integer: "Integer", Numeric: "Numeric", String: "String", Boolean: "Boolean", Array: "Array", Object: "Object", Any: "" };
class b {
  static typeOf(e2) {
  ...
  static unwrap(e2, t2 = _.Any) {
    let o2 = this.typeOf(e2), a2 = Object.values(e2)[0];
    switch (o2) {
      case "Object":
        a2 = Object.values(a2).map(([e3, t3]) => [this.unwrap(e3, _.String), this.unwrap(t3)]), a2 = Object.fromEntries(a2);
        break;
      case "Array":
        a2 = a2.map((e3) => this.unwrap(e3));
    }
    return b.cooerce(a2, t2);
  }
  static wrap(e2, t2 = _.Any) {
...
class w {
  constructor(e2, t2, o2) {
    this.name = e2, this.callback = o2, this.argumentTypes = [], this.returnType = t2, this.registeredName = w.getRegisteredName(e2);
  }
  static isStateAvailable() {
    return "function" == typeof getState;
  }
  static getRegisteredName(e2) {
    return `lavendeuxfunction_${e2}_${(Math.random() + 1).toString(36).substring(8)}`;
  }
  addArgument(e2 = _.Any) {
    return this.argumentTypes.push(e2), this;
  }
  addIntegerArgument() {
    return this.addArgument(_.Integer);
  }
  addFloatArgument() {
    return this.addArgument(_.Float);
  }
  addNumericArgument() {
    return this.addArgument(_.Numeric);
  }
  addStringArgument() {
    return this.addArgument(_.String);
  }
  addBooleanArgument() {
    return this.addArgument(_.Boolean);
  }
  addArrayArgument() {
    return this.addArgument(_.Array);
  }
  addObjectArgument() {
    return this.addArgument(_.Object);
  }
  decodeArguments(e2) {
    ...
    return this.argumentTypes.forEach((t2, o2) => {
      let a2 = b.typeOf(e2[o2]);
      if (t2 == _.Numeric && ![_.Integer, _.Float].includes(a2) || [_.Integer, _.Float].includes(t2) && t2 != a2)
        throw new Error(`Invalid value for parameter ${o2 + 1} of ${this.name}: Expected ${e2[o2].type}`);
    }), e2.map((t2, o2) => {
      let a2 = e2[o2] ? e2[o2] : _.Any;
      return b.unwrap(t2, a2);
    });
  }
  getState() {
    return w.isStateAvailable() ? getState() : {};
  }
  setState(e2) {
    w.isStateAvailable() && setState(e2);
  }
  call(e2) {
    e2 = this.decodeArguments(e2);
    let t2 = this.getState(), o2 = b.wrap(this.callback(...e2), this.returnType);
...
class k {
  ...
  addIntegerFunction(e2, t2) {
    return this.addFunction(e2, t2, _.Integer);
  }
  addFloatFunction(e2, t2) {
    return this.addFunction(e2, t2, _.Float);
  }
  addNumericFunction(e2, t2) {
    return this.addFunction(e2, t2, _.Numeric);
  }
  addStringFunction(e2, t2) {
    return this.addFunction(e2, t2, _.String);
  }
  addBooleanFunction(e2, t2) {
    return this.addFunction(e2, t2, _.Boolean);
  }
  addArrayFunction(e2, t2) {
    return this.addFunction(e2, t2, _.Array);
  }
  addObjectFunction(e2, t2) {
    return this.addFunction(e2, t2, _.Object);
  }
  addDecorator(e2, t2, o2 = _.Any) {
    let a2 = new w(e2, _.String, t2);
    return a2.addArgument(o2), this.allHandlers[a2.registeredName] = a2, this.decorators[e2] = a2.registeredName, a2;
  }
  addIntegerDecorator(e2, t2) {
    this.addDecorator(e2, t2, _.Integer);
  }
  addFloatDecorator(e2, t2) {
    this.addDecorator(e2, t2, _.Float);
  }
  addNumericDecorator(e2, t2) {
    this.addDecorator(e2, t2, _.Numeric);
  }
  addStringDecorator(e2, t2) {
    this.addDecorator(e2, t2, _.String);
  }
  addBooleanDecorator(e2, t2) {
    this.addDecorator(e2, t2, _.Boolean);
  }
  addArrayDecorator(e2, t2) {
    this.addDecorator(e2, t2, _.Array);
  }
  addObjectDecorator(e2, t2) {
    this.addDecorator(e2, t2, _.Object);
  }
}
class A extends v {
  constructor(e2) {
  ...
  draw() {
    ...
    return ["", ...f.getTitledBox(e2.title, e2.description), this.error ? "\nInvalid option!" : "", "What do you do?", ...e2.options.map((e3, t2) => `${t2 + 1} @zarban) ${e3}`), "", 'You can choose an option from above, such as "1 @zarban" and "zarban(2)" or start a new game with "start/restart @zarban" or "zarban()"', "Type your selection below and use Lavendeux to continue your adventure!\n"].join("\n");
  }
  static registerExtension(e2, t2, o2) {
    let a2 = new k(e2, t2, o2);
    a2.addStringDecorator("zarban", A.callback), a2.addStringDecorator("Zarban", A.callback), a2.addStringFunction("zarban", A.callback).addStringArgument(), a2.addStringFunction("Zarban", A.callback).addStringArgument(), k.register(a2);
  }
  static callback(e2, t2) {
    ["start", "restart", ""].includes(e2.toLowerCase()) && delete t2.zarban_save;
    const o2 = new A(t2.zarban_save), a2 = o2.step(!!t2.zarban_save && e2);
    return t2.zarban_save = o2.save(), a2;
  }
}
A.registerExtension("zarbans_grotto", "@rscarson", "1.0.0");
class x {
  constructor(e2) {
...
const T = "zarban_gameboard", j = "zarban_controls";
class S extends v {
  constructor(e2) {
  ...
  initContainer() {
    this.container.dataZarbanRunner = this, this.container.innerHTML = "", this.container.appendChild(new x("div").setAttribute("class", T).element()), this.container.appendChild(new x("div").setAttribute("class", j).element());
  }
  injectCSS() {
  ...
  draw() {
    const e2 = this.getInterfaceStrings(), t2 = this.container.getElementsByClassName(T)[0], o2 = this.container.getElementsByClassName(j)[0];
    e2.description = e2.description.map((e3) => 0 == e3.length ? "<br/>" : e3), t2.innerHTML = "", t2.appendChild(new x("a").setAttribute("href", "#").setStyle("float", "right").setInnerHTML("[ New Game ]").setOnClick(() => this.reset()).element()), t2.appendChild(new x("h4").setInnerHTML(e2.title).element()), e2.description.map((e3) => {
      t2.appendChild(new x("p").setInnerHTML(e3).element());
    });
    const a2 = document.createElement("p");
    a2.innerHTML = "What do you do?<br/>";
    for (const t3 in e2.options)
      a2.appendChild(new x("a").setAttribute("href", "#").setInnerHTML(`> ${e2.options[t3]}`).setOnClick(() => this.step(parseInt(t3) + 1)).element());
...
export {
  A as ZarbanLavendeuxRunner,
  S as ZarbanWebRunner
};
